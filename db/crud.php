      ...
      public function num(){
       ...
       public function data($user){
          ...
          try{
            ...
            $stmt = $this->db->prepare($sql);  $stmt->bindparam(':user', $user);
            $stmt->execute();
            $result = $stmt->fetch();
            return $result;
       }catch (PDOException $e) {
       ...
       public function delete($usr){
        try{
         ...
         try{ 
          $sql = "UPDATE `sign` SET `mobile`=:mbl, `fullname`=:nam, `country`=:cntry, `bdate`=:dt,
           `img_path`=:img, `ides`=:ides WHERE email=:em";
          $stmt = $this->db->prepare($sql);
          $stmt->bindparam(':nam',$n); $stmt->bindparam(':mbl',$mb);
          $stmt->bindparam(':dt',$bd); $stmt->bindparam(':cntry',$cn);
          $stmt->bindparam(':em',$em); $stmt->bindparam(':ides',$ides);
...
public function hstry($id){
  ...
  public function insertcrs($title,$descrip,$destination,$cat,$tname,$tid,$lrn,$cdes){
    try {
     $sql = "INSERT INTO course (c_name,category, des,  img_path,tid, tname,learn, cdes	) 
     VALUES (:title, :cat, :descrip,  :img,:tid, :tname,:lrn, :cdes)";
     $stmt = $this->db->prepare($sql); $stmt->bindparam(':title',$title);
     $stmt->bindparam(':cat',$cat); $stmt->bindparam(':descrip',$descrip);
     $stmt->bindparam(':img',$destination); $stmt->bindparam(':tid',$tid);
     $stmt->bindparam(':tname',$tname); $stmt->bindparam(':lrn',$lrn);
  ...
  public function insertsec($title,$id,$destination){
    try {
     ...
     $stmt = $this->db->prepare($sql);
     $stmt->bindparam(':title',$title); $stmt->bindparam(':id',$id);
   ...
   public function insertqus($user,$qs,$cid){
    try {
     ...
     $stmt = $this->db->prepare($sql); $stmt->bindparam(':user',$user);  $stmt->bindparam(':ques',$qs); $stmt->bindparam(':cid',$cid);
     $stmt->execute();
     return true;
    } catch (PDOException $e) {
   ...
   public function qustn($id){
    ...
    public function insertans($id,$ans,$cid){
     try{ 
      ...
      $stmt = $this->db->prepare($sql);
      $stmt->bindparam(':ans',$ans); $stmt->bindparam(':aid',$id); $stmt->bindparam(':crid',$cid);
      $stmt->execute();
      return true;
      } catch (PDOException $e) {
        echo $e->getMessage(); return false;
   ...
   public function insertbooks( $title,$author,$cat,$pdf,$pub,$ed,$lang,$destination,$size){
    try {
     $sql = "INSERT INTO books (bname,aname,bcat,pdf,publshr,edtn,lang,img_path,size) 
     VALUES (:title, :author, :cat, :pdf, :pub, :ed, :lang, :destination, :size)";
     $stmt = $this->db->prepare($sql);
     $stmt->bindparam(':title',$title);
...
public function viewcrct($id,$qn,$lv){
  try {
   ...
   $stmt = $this->db->prepare($sql);
   $stmt->bindparam(':id',$id);  $stmt->bindparam(':lv',$lv); $stmt->bindparam(':qn',$qn);
   $stmt->execute();
   $result = $stmt->fetch(); 
   return $result;
 } catch (PDOException $e) {
 ...
 try{ 
  ...
  $stmt = $this->db->prepare($sql); $stmt->bindparam(':cid',$id); $stmt->bindparam(':vdo',$destination);
  $stmt->execute();
  return true;
  } catch (PDOException $e) {
    echo $e->getMessage(); return false;
...
public function deleteimg($destination, $id){
  try{
       ...
       $stmt = $this->db->prepare($sql);
       $stmt->bindparam(':img', $destination); $stmt->bindparam(':id', $id);
       $stmt->execute();
       return true;
   }catch (PDOException $e) {
       echo $e->getMessage();  return false;
...
public function deletequs($qus, $id){
  try{
       ...
       $stmt = $this->db->prepare($sql);
       $stmt->bindparam(':qus', $qus);  $stmt->bindparam(':id', $id);
       $stmt->execute();
       return true;
   }catch (PDOException $e) {
       echo $e->getMessage(); return false;
...
public function updtimg($id,$destination){
  try{ 
   $sql = "INSERT INTO books (bname,aname,bcat,pdf,publshr,edtn,lang,img_path) 
   VALUES (:title, :author, :cat, :pdf, :pub, :ed, :lang, :destination)";   
   $stmt = $this->db->prepare($sql);
   $stmt->bindparam(':cid',$id);
   $stmt->bindparam(':vdo',$destination);
   $stmt->execute();
